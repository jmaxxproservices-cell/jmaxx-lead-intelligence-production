...
export async function getLeadStats(): Promise<{
  ...
  const validStatuses: LeadStatus[] = ['new', 'contacted', 'quoted', 'won', 'lost'];
  const validClassifications = ['hot', 'high', 'medium', 'low'] as const;

  data.forEach((item) => {
    const status = item.status as string;
    if (validStatuses.includes(status as LeadStatus)) {
      stats[status as LeadStatus]++;
    }
    const classification = item.classification as string;
    if (classification && validClassifications.includes(classification as typeof validClassifications[number])) {
      stats[classification as 'hot' | 'high' | 'medium' | 'low']++;
    }
  });
